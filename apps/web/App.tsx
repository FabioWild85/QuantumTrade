...
function saveReportCache(symbol: AssetSymbol, report: MarketReport) {
...
function loadReportCache(symbol: AssetSymbol): MarketReport | null {
  try {
    const raw = sessionStorage.getItem(`report_cache_${symbol}`);
    if (!raw) return null;
    const { report, timestamp } = JSON.parse(raw);
    if (Date.now() - timestamp > REPORT_CACHE_TTL_MS) return null;
    return report as MarketReport;
  } catch {
    return null;
  }
}
...
const DashboardHome: React.FC<{ isDarkMode: boolean; toggleTheme: () => void }> = ({ isDarkMode, toggleTheme }) => {
  const navigate = useNavigate();
  const [forceRefreshMode, setForceRefreshMode] = useState<boolean>(false);

  const handleSelectAsset = (symbol: AssetSymbol) => {
    if (forceRefreshMode) {
      sessionStorage.removeItem(`report_cache_${symbol}`);
    }
    navigate(`/analysis/${symbol.toLowerCase()}`);
  };
          ...
          <div className="w-full max-w-5xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-4xl mx-auto">
              ...
              >
                <div className="flex items-center justify-between mb-6">
                  <div className="p-4 bg-orange-50 dark:bg-orange-500/10 rounded-2xl group-hover:scale-110 transition-transform duration-300">
                    <img src="https://upload.wikimedia.org/wikipedia/commons/4/46/Bitcoin.svg" alt="Bitcoin" className="w-10 h-10" />
                  </div>
                ...
                <p className="text-slate-500 dark:text-slate-400 text-sm text-left leading-relaxed">Dominance, Order Flow e cicli di Halving.</p>
              </div>
              ...
              >
                ...
                <p className="text-slate-500 dark:text-slate-400 text-sm text-left leading-relaxed">Staking Metrics e correlazione DeFi.</p>
              </div>
              ...
              >
                <div className="flex items-center justify-between mb-6">
                  <div className="p-4 bg-gradient-to-br from-[#9945FF]/10 to-[#14F195]/10 rounded-2xl group-hover:scale-110 transition-transform duration-300">
                    <img src="https://upload.wikimedia.org/wikipedia/en/b/b9/Solana_logo.png" alt="Solana" className="w-10 h-10 object-contain" />
                  </div>
                ...
                <p className="text-slate-500 dark:text-slate-400 text-sm text-left leading-relaxed">High Throughput e Momentum Analysis.</p>
              </div>
            </div>
...
const AnalysisReport: React.FC<{ isDarkMode: boolean; toggleTheme: () => void }> = ({ isDarkMode, toggleTheme }) => {
  const { asset } = useParams<{ asset: string }>();
  const navigate = useNavigate();

  const symbol = asset?.toUpperCase() as AssetSymbol;
  const isValidAsset = VALID_ASSETS.includes(symbol);
  ...
  // On mount: try to restore from cache, otherwise auto-generate
  useEffect(() => {
    if (!isValidAsset) return;
    const cached = loadReportCache(symbol);
    if (cached) {
      setReport(cached);
    } else {
  ...
  const handleGenerateAnalysis = useCallback(async (forceRefresh: boolean = false) => {
    if (!isValidAsset) return;
    setLoading(true);
    setError(null);
  ...
  const handleExport = async () => {
    ...
    if (element) {
      try {
        ...
        link.download = `QuantAI_Analysis_${symbol}_${new Date().toISOString().split('T')[0]}.png`;
  ...
  return (
    <div className="min-h-screen font-sans transition-colors duration-300 bg-[#F8FAFC] dark:bg-[#0B1120] text-slate-900 dark:text-slate-200 w-full overflow-x-hidden flex flex-col selection:bg-indigo-500/20">
      ...
      <main className="flex-grow container mx-auto px-4 md:px-6 py-10 max-w-[1400px] pb-32">
        ...
        {/* Loading */}
        {loading && (
          <LoadingScreen asset={symbol} step={loadingStep} isDarkMode={isDarkMode} />
        )}
        ...
        {error && !loading && (
          <div className="max-w-lg mx-auto mt-20 p-8 bg-white dark:bg-[#151E32] rounded-2xl border border-red-100 dark:border-red-500/30 shadow-xl text-center animate-fade-in">
            ...
            <div className="flex gap-3 justify-center">
              <button
                onClick={() => navigate('/')}
                className="px-6 py-3 bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-300 font-semibold rounded-xl hover:opacity-90 transition-opacity"
              >
                Home
              </button>
              <button
                onClick={() => { setError(null); handleGenerateAnalysis(true); }}
        ...
        {report && !loading && (
          <div className="space-y-8 animate-fade-in" id="report-container">
            ...
            <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 bg-white dark:bg-[#151E32] p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-white/5">
              <div className="w-full lg:w-auto">
                <h2 className="text-2xl font-bold text-slate-900 dark:text-white flex items-center gap-3">
                  {symbol === 'BTC' && <img src="https://upload.wikimedia.org/wikipedia/commons/4/46/Bitcoin.svg" alt="BTC" className="w-8 h-8" />}
                  {symbol === 'ETH' && <svg className="w-8 h-8 text-blue-600 dark:text-blue-400" viewBox="0 0 24 24" fill="currentColor"><path d="M12 1.75l-6.13 9.25 6.13 3.42 6.13-3.42L12 1.75z m0 15.5l-6.13-3.42 6.13 9.42 6.13-9.42-6.13 3.42z"/></svg>}
                  {symbol === 'SOL' && <img src="https://upload.wikimedia.org/wikipedia/en/b/b9/Solana_logo.png" alt="Solana" className="w-8 h-8 object-contain" />}
                  Executive Report
                </h2>
            ...
            <MarketSummary data={report.technical} macroSentiment={report.macroSentiment} macroOverview={report.macroOverview} />

            {report.trendReversal && (
              <TrendReversal bullish={report.trendReversal.bullishScenario} bearish={report.trendReversal.bearishScenario} />
            )}

            <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
              <div className="xl:col-span-2 flex flex-col gap-8 min-w-0">
                <TechnicalPanel technical={report.technical} asset={symbol} />
