...
export const TradingHubTab: React.FC<TradingHubTabProps> = ({ onBackToDashboard }) => {
  const [page, setPage] = useState<HubPage>('monitor');
  const [isCollapsed, setIsCollapsed] = useState<boolean>(false);
  const [isMobileOpen, setIsMobileOpen] = useState<boolean>(false);

  // Close mobile drawer on resize or path change
  useEffect(() => {
  ...
  const renderSidebarContent = () => (
    <div className="h-full flex flex-col justify-between py-6">
      ...
      <div className="space-y-4 px-6">
        <div className={`flex items-center ${isCollapsed ? 'justify-center' : 'justify-between'} border-t border-slate-100 dark:border-white/5 pt-4`}>
